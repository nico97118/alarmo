...
class AlarmoBaseEntity(AlarmControlPanelEntity, RestoreEntity):
    ...
    async def _validate_code(self, code, to_state):
        ...
        def _is_code_required():
            ...
            if (
                ...
                _LOGGER.debug(
                    "Code is not required for arming, bypassing code validation."
                )
                return False
        ...
        def _is_user_allowed_for_area(user):
        ...
        def _is_user_allowed_for_action(user):
        ...
        async def _resolve_user():
            ...
            if not code or len(code) < 1:
                _LOGGER.error("No code provided, cannot resolve user.")
                return None
        ...
        if not _is_user_allowed_for_area(user):
            return False, const.EVENT_INVALID_CODE_PROVIDED

        # action permission check
        if not _is_user_allowed_for_action(user):
            return False, const.EVENT_INVALID_CODE_PROVIDED
